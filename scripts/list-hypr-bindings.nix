...
pkgs.writeShellScriptBin "list-hypr-bindings" ''
  ...
  " = Mod/Super/" "Modifier Key, used for keybindings" "Doesn't really execute anything by itself." \
  "fr" "Rebuild switch --flake" "${terminal}" \
  "ncg" "Remover versões" "${terminal}" \
  "fu" "nix flake update" "${terminal}" \
  "zu" "Atualização ZaneyOS" "${terminal}" \
  " + T" "Terminal" "${terminal}" \
  " + A" "App Launcher" "rofi" \
  ...
  " + B" "Launch Web Browser" "zen-browser" \
  " + Z" "Launch Web Browser" "vivaldi" \
  " + Return" "Launch Emoji Selector" "emopicker9000" \
  ...
  " + W" "Toggle Focused Fullscreen" "fullscreen" \
